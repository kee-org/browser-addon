...
<!-- :elevation="{hover ? 12 : 3}" -->
        <v-card
ref="card" v-slot="{ hover }" :tabindex="`${tabindex}`" class="my-2"
            :ripple="false" @focusin="focusin" @focusout="focusout" @keyup.context-menu.stop.prevent="showFullDetails"
            @keyup.arrow-down.stop.prevent="nextInList" @keyup.arrow-up.stop.prevent="prevInList"
            @keyup.arrow-right.stop.prevent="showFullDetails" @keyup.arrow-left.stop.prevent="hideFullDetails"
            @keyup.escape.stop.prevent="exitList" @keyup.enter.self.stop.prevent="primaryClickAction"
            @keyup.enter.ctrl.self.stop.prevent="loadInNewTab">
            <v-container class="ma-0 pa-0 my-2" :style="titleStyle">
                <v-row
class="mr-3 ml-12 my-0" :style="`${hover ? 'cursor: pointer' : ''}`"
                    @click.left.exact="primaryClickAction" @click.middle.exact="loadInNewTab"
                    @click.left.ctrl="loadInNewTab" @click.middle.ctrl="primaryClickAction"
...
:style="`${expanded ? 'visibility:hidden' : ''}`"
                    ...
                    <v-col v-if="hover || focussed" cols="2" class="mr-3 align-self-center">
                        <v-btn icon @click.stop.prevent="toggleFullDetails">
                            <mdi-chevron-down v-if="!expanded" />
                        ...
                        <v-row justify="space-between" align="center" class="my-0">
                            <v-col class="text-truncate">
                                <v-row class="justify-left text-truncate flex-nowrap my-0" align="center">
                                    <v-tooltip
location="top" :disabled="!entryPathIsLong" :open-delay="tooltipDelay"
                                        activator="parent">
                                        <span>{{ fullEntryPath }}</span>
                                    </v-tooltip>
                                    <mdi-folder size="small" class="py-1 pl-0 pr-2" />
                                ...
                                <v-row class="justify-left text-truncate flex-nowrap my-0" align="center">
                                    ...
                                    <mdi-cloud size="small" class="py-1 pl-0 pr-2" />
                                </v-row>
                            </v-col>
