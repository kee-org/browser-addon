...
export class Group {
    ...
    public static fromKPRPCGroupDTO (groupDto: GroupDto, dbFileName: string) {
        return new Group({
            ...
            path: groupDto.path,
            entrySummaries: groupDto.childLightEntries.map(childLightEntry => EntrySummary.fromKPRPCEntrySummaryDTO(childLightEntry, groupDto.path, dbFileName)),
            groups: groupDto.childGroups.map(childGroup => this.fromKPRPCGroupDTO(childGroup, dbFileName))
        });
    }
}
