class ConfigMigrations {
    ...
    public migrateToVersion3 (current: Config) {
        ...
        Object.assign(current, { currentSearchTermTimeout: 30, version: 3 } as Partial<Config>);
    }
    ...
    public migrateToVersion2 (current: Config) {
        ...
        for (let i=1; i<current.config.length; i++) // skip first which is always "*"
        {
            ...
            const newConfig = this.migrateIndividualSiteConfigSettingsToV2(current.config[i].config);
        ...
        Object.assign(current, { siteConfig: newSiteConfig, config: null, version: 2 } as Partial<Config>);
    }
