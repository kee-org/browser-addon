...
function loadInitialConfig() {
    ...
    ) as HTMLInputElement).checked = configManager.current.autoSubmitNetworkAuthWithSingleMatch
    ...
    ) as HTMLInputElement).checked = configManager.current.rememberMRUDB
        ? configManager.current.rememberMRUDB
        : null;

    (document.getElementById(
        "pref_when_kee_chooses_standard_form_desc"
    ) as HTMLSelectElement).value = configManager.current.autoSubmitForms
        ? "FillAndSubmit"
    ...
    ) as HTMLSelectElement).value = configManager.current.autoSubmitMatchedForms
        ? "FillAndSubmit"
        : "Fill";
    ...
    ) as HTMLInputElement).value = configManager.current.currentSearchTermTimeout
...
function saveSearchAllOpenDBs(e) {
    ...
    configManager.setASAP({
        searchAllOpenDBs: (document.getElementById(
            "pref_searchAllOpenDBs_label"
...
function saveManualSubmitOverrideProhibited(e) {
    e.preventDefault();
    configManager.setASAP({
        manualSubmitOverrideProhibited: !(document.getElementById(
            "pref_manualFillSubmitOverridable_label"
        ) as HTMLInputElement).checked
    });
}
...
function saveAutoSubmitNetworkAuthWithSingleMatch(e) {
    e.preventDefault();
    configManager.setASAP({
        autoSubmitNetworkAuthWithSingleMatch: (document.getElementById(
            "pref_when_one_matching_network_login"
        ) as HTMLInputElement).checked
    });
}
